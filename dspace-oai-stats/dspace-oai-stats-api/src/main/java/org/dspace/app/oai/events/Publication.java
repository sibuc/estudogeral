...
public class Publication extends Event {
	// private Logger logger = Logger.getLogger(Publication.class);
	private Item item = null;

	public Publication(String table) {
		super(null);
	}
	...
	public String getDate() {
		DCDate date = new DCDate(EventsUtils.getFirstValueIfNotNullOrEmpty(item.getMetadata("dc", "date", "issued", Item.ANY)));
		return Event.simpleDateFormat.format(date.toDate());
	}
	...
	public String serialize2XML() {
		...
		try {
			...
			if (item != null) {
				...
				String type = null, rights = null, language = null, date_issued = null;
				...
				// event unique id
				contextObject.setIdentifier(DSpaceOAICatalog.OAI_ID_PREFIX + "publication_" + getPublication_id());
				// event generation date
				contextObject.setTimestamp(date_issued);
				...
				EventsUtils.addDcTermMetadataToDescriptorType(serviceTypeDescriptorType, new Format("info:eu-repo/semantics/publication"));
