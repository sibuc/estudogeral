...
public abstract class DSpaceObject
{
    ...
    public Metadatum[] getMetadataByMetadataString(String mdString)
    {
        ...
        // log.info("getMetadataByMetadataString");
        String[] tokens = { "", "", "" };
        int i = 0;
       ...
       // log.info("getMetadataByMetadataString n.= " + values.length );
        return values;
    }
                            ...
                            String[] values, String authorities[], int confidences[])
    {
        ...
        // log.info("addMetadata.length = " + values.length);
        for (int i = 0; i < values.length; i++)
        {
            Metadatum dcv = new Metadatum();
            dcv.schema = schema;
            //log.info("addMetadata.schema = " + dcv.schema);
            dcv.element = element;
            //log.info("addMetadata.element = " + dcv.element);
            dcv.qualifier = qualifier;
           //log.info("addMetadata.qualifier = " + dcv.qualifier);
            ...
            if (values[i] != null)
            {
                ...
                log.info("addMetadata.value = " + dcv.value);
            }
            else
            {
                dcv.value = null;
    ...
    protected String[] getMDValueByLegacyField(String field){
        switch (field) {
            ...
            case "preferredhandles":    
                return new String[]{"eperson","preferredhandles",null};
            case "embargo":
                return new String[]{"collection","embargo",null};           
            case "inicioprazodeposito":
                return new String[]{"collection","inicioprazodeposito",null};
            default:
