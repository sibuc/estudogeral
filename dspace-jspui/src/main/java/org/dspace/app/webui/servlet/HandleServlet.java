...
public class HandleServlet extends DSpaceServlet {
    ...
    protected void doDSGet(Context context, HttpServletRequest request,
        ...
        if ("/statistics".equals(extraPathInfo)) {
            ...
            // and firing a usage event for the DSO we're reporting for
            return;
        } else if ("/finalizaprovas".equals(extraPathInfo)) {
            // Check configuration properties, auth, etc.
            // Inject handle attribute
            log.info(LogManager.getHeader(context, "/finalizaprovas", "handle=" + handle + ", path=" + extraPathInfo));
            request.setAttribute("handle", handle);

            // Forward to FinalizaProvasServlet without changing path.
            RequestDispatcher dispatch = getServletContext().getNamedDispatcher("finalizaprovas");
            dispatch.forward(request, response);
        ...
        if (dso.getType() == Constants.ITEM) {
             ...
             */
            int comm_doutoramentos = ConfigurationManager.getIntProperty("comm.doutoramentos");
            Community[] parents = c.getCommunities();
            request.setAttribute("dspace.community", parents[0]);
            boolean finalizaprovas = false;
            boolean doutoramentos = false;

            if (comm_doutoramentos == parents[0].getID()) {

                doutoramentos = true;
                int grupo_provas = ConfigurationManager.getIntProperty("academicos.grupo.provas");
                Group grupo = Group.find(context, grupo_provas);
                log.info("Grupo-provas:" + grupo_provas);
                if (grupo.isMember(context.getCurrentUser())) {
                    finalizaprovas = true;
                    log.info("Finaliza_provas:" + finalizaprovas);
                }
                }
                request.setAttribute("doutoramentos", doutoramentos);
                request.setAttribute("finalizaprovas", finalizaprovas);
                /*
     ...
     */
    public void collectionHome(Context context, HttpServletRequest request,
            HttpServletResponse response, Community community,
        ...
        if (comm_doutoramentos == collection.getParentObject().getID()) {
            doutoramentos = true;
        }
        request.setAttribute("doutoramentos", doutoramentos);
        // Handle click on a browse or search button
