...
%>
            ...
            <a href="<%= sharedLink %>
            <% if (results[i][1] != null) { %>&amp;authority=<%= URLEncoder.encode(results[i][1], "UTF-8") %>&amp;value=<%= URLEncoder.encode(results[i][0], "UTF-8") %>" class="authority <%= bix.getName() %>"><%= Utils.addEntities(results[i][0]) %></a> <% } 
