...
<%
    ...
    Integer com_dout = (Integer) ConfigurationManager.getIntProperty("comm.doutoramentos");
    Community[] communities =(Community[]) item.getCommunities();
    boolean doutoramento = false;
    ...
    // controlo do botao de pedido de copia para doutoramentos
    for (int l=0; l< communities.length; l++)
     {
        if (communities[l].getID() == com_dout) {
           doutoramento=true;
           break;
        }
     }
...
%>
	<!--STATS ADDON  -->
		<%
			...
			{
			%>
				<a class="statisticsLink  btn btn-info" href="<%= request.getContextPath() %>/stats?level=item&type=access&page=downviews-series&object=item&object-id=<%= handle %>">
...
%>
    <a href="<%= request.getContextPath() %>/pedir-copia?item-handle=<%= handle %>">
       <fmt:message key="jsp.layout.navbar.pedido-copia"/></a>
<%
        }
%>
...
%>
<br/>
<% if (ConfigurationManager.getBooleanProperty("sharing.active", false)  && !isAdmin) { %>
	<% // code
