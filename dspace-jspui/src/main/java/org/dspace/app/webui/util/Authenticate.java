...
public class Authenticate
{
                                ...
                                EPerson eperson)
    {
        ...
        // Identificacao de que se trata de um utilizador dos servicos academicos e
        // tem disponivel a opcao de "criar provas"
        ...
        {
          if (eperson!=null) {
           Integer grupo = (Integer) ConfigurationManager.getIntProperty("academicos.grupo.provas");
           Group grp = Group.find(context, grupo);
           log.info("Atribui academicos");
           log.info("Grupo:" + grupo.toString());
           log.info("grp:" + grp.toString());
           if (grp != null) {
            academicos = grp.isMember(eperson);
           }
          }
        }
        catch (SQLException se1)
        {
            log.warn("Unable to use AuthorizeManager " + se1);
        }
        finally 
        {
            request.setAttribute("academicos", Boolean.valueOf(academicos));
        }
