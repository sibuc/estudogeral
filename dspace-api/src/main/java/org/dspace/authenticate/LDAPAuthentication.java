...
/** Criação de utilizadores candidatos a provas academicas
 *  Devolve o utilizador que deverá ser criado para não haver problemas de permissoes
 */
    
    public static String academicos_authenticate(Context context,
                            String netid,
                            String password,
                            String realm,
                            HttpServletRequest request)
        throws SQLException
    {
        ...
        if (eperson != null)
        {
            ...
            log.info("eperson.getNetid = " + ldap.ldapEmail);
            return ldap.ldapEmail;
        }
        else
        {
         // the user does not already exist 
         // return netid based on dn
        //    String uid = null;
        //    String sufixo = null;
        //    String[] ldap_login = dn.split(Pattern.quote(","));
        //    log.info("Length de ldap_login - "+ ldap_login.length);
        //    for (int i=0; i<ldap_login.length; i++) {
        //        log.info("ldap_login(" + i + ")="+ ldap_login[i]);
        //        if (ldap_login[i].contains("uid")) uid = ldap_login[i].substring(ldap_login[i].indexOf("uid=")+4, ldap_login[i].length());
        //        if (ldap_login[i].contains("dc"))
        //            if (sufixo == null)
        //                sufixo = ldap_login[i].substring(ldap_login[i].indexOf("dc=")+3, ldap_login[i].length());
        //            else
        //               sufixo = sufixo.concat(".").concat(ldap_login[i].substring(ldap_login[i].indexOf("dc=")+3, ldap_login[i].length()));
        //          
        //    }
            
            email = ldap.ldapEmail;
                    ...
                    while (answer.hasMoreElements()) {
                        ...
                        if (attlist[0] != null) {
                            ...
                            if (att != null)
                            {
                                ...
                                log.info("ldapEmail (" + i++ + ")" + ldapEmail);
                            }
                        }
