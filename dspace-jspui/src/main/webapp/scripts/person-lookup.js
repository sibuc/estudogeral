...
function AuthorLookup(url, authorityInput, collectionID, flag) {
//    TODO i18n
...
if (flag=="Name")
{
     nameMessage = "Name";
	 loadMessage = "Loading...";
	 noneMessage = "There\'s no one selected";
	 itemMessage = "Items in this repository:&nbsp;";
     addPersonMessage = "Add This Person";
	 noPersonMessage = "No people found";
	 ...
	 showSearch = "Search:";
	 moreMessage = "more results";
    lessMessage = "less results";
}
else
{

     nameMessage = getNameMessage();
	 loadMessage = getLoadMessage();
	  noneMessage = getNoneMessage();
	  itemMessage = getItemMessage();
      addPersonMessage = getAddPersonMessage();
	  noPersonMessage = getNoPersonMessage();
      resultsMessage = getResultsMessage();
	  showMessage = getShowMessage();
 	  showFirstName = getFirstNameMessage();
	  showLastName =  getLastNameMessage();
	  showProcessing =  getProcessingMessage();
	  showSearch = getSearchMessage();
	  moreMessage = getMoreMessage();
      lessMessage = getLessMessage();
	 }
...
//									'<button class="close" data-dismiss="modal" type="button">'+
                                ...
                                '<div class="modal-body">'+
                                    '<div title="Person Lookup">' +
                                        '<table class="dttable col-xs-4">' +
                                            ...
                                            '</thead>' +
                                            '<tbody>' +
                                                '<tr><td>'+loadMessage+'<td></tr>' +
                                            '</tbody>' +
                                        '</table>' +
                                        '<span class="no-vcard-selected">'+noneMessage+'</span>' +
                                        '<ul class="vcard list-group" style="display: none;">' +
                                            ...
                                            '<li class="vcard-add list-group-item">' +
                                                '<input class="ds-button-field btn btn-default" value="'+addPersonMessage+'" type="button"/>' +
                                            '</li>' +
                                        '</ul>' +
                                    '</div>'+
                                '</div>'+
	...
	//alert(myMessage);
    var moreButton = '<button id="lookup-more-button" class="btn btn-default" name="morebt">'+ moreMessage +'</button>';
   // var moreButton = '<button id="lookup-more-button" class="btn btn-default" name="morebt">show more</button>';
    var lessButton = '<button id="lookup-less-button" class="btn btn-default" name="morebt">'+ lessMessage +'</button>';
...
//			var searchlabel = content.find('.dataTables_filter label');
//			searchlabel.replaceWith("<label>Procurar:<input class='form-control' placeholder='' aria-controls='DataTables_Table_0' type='search'></label>");
			...
			//ARBT
                var lastName = $('input[name=text1]'); //$('input[name=' + authorityInput + '_last]');
                if (lastName.size()) { // author input type
//if (lastName.val().trim()=="")  {lastName.val() = lastName.val() +  ", "}
                   // if (lastName.val()!="") lastName.val() = lastName.val() + ", " ;

					if (lastName.val()!="")
//					
					{
                    initialInput = (lastName.val() + ", " + $('input[name=text2').val()).trim();  //ARBT (lastName.val() + " " + $('input[name=' + authorityInput + '_first]').val()).trim();
					}
					 //initialInput = ($('input[name=text2').val()).trim() + " " + lastName.val();
		 ...
		 //var searchLabel = content.find('.dataTables_filter label');
		 //searchLabel.val="tewss";
		 
		//arbt	 
		var searchProcessing = content.find('.dataTables_processing');
    	searchProcessing.html(showProcessing);
		
		var searchlabel = content.find('.dataTables_filter label span');
		searchlabel.replaceWith(showSearch);

	//var searchlabel = content.find('.dataTables_filter label');
		//var trad = (searchlabel.text()).replace("Search:", "Procurar:"); 
		//var searchFilter2 = content.find('.dataTables_filter input');
		//searchlabel.text(trad + <input class="form-control" placeholder="" aria-controls="DataTables_Table_0" type="search">);
		


		//searchlabel.last().replaceWith('my new text');
//searchlabel.replaceWith("<div><p>"+"teste"+"</div>");
//searchlabel.replaceWith("<input class='form-control' placeholder='' aria-controls='DataTables_Table_0' type='search'>");




		//alert(searchFilter2.innerHTML);
		//alert(searchFilter2.text());
		//alert(searchFilter2.val());
		//alert (trad);
		//;	
		//alert(searchlabel.text());
    	//alert(searchlabel.text("Procurar" + searchFilter.innerhtml() ));

        },
		
        "fnInfoCallback": function( oSettings, iStart, iEnd, iMax, iTotal, sPre ) {
          //return "Showing "+ iEnd + " results. "+button;
		   return (showMessage + " " + iEnd + " " + resultsMessage + ". "+button);
        ...
        "fnRowCallback": function( nRow, aData, iDisplayIndex ) {
            ...
            $row.click(function() {
                ...
                function variableItem(aData, key, variable) {
					...
					if (label == "last name") {label=showLastName;}
					if (label == "first name") {label=showFirstName;}
                ...
                if(aData['insolr']!="false"){
				   ...
				   var discoverLink = "https://193.137.200.162/jspui/simple-search?location=&query=&filtername=author&filtertype=authority&filterquery=" + aData['insolr'].trim() + "				   &rpp=10&sort_by=score&order=desc";
