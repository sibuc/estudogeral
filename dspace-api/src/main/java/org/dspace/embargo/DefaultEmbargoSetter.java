...
public class DefaultEmbargoSetter implements EmbargoSetter
{
        ...
        throws SQLException, AuthorizeException, IOException
    {
        ...
        log.info("liftDate:" + liftDate.displayDate(true, true, Locale.ENGLISH) );
        for (Bundle bn : item.getBundles())
        {
            ...
            if (!(bnn.equals(Constants.LICENSE_BUNDLE_NAME) || bnn.equals(Constants.METADATA_BUNDLE_NAME) || bnn.equals(CreativeCommons.CC_BUNDLE_NAME)))
            {
                ...
                log.info("removePoliciesActionFilter");
    ...
    protected void generatePolicies(Context context, Date embargoDate,
        ...
        log.info("generatePolicies - embargodate " + embargoDate.toGMTString());
        if(embargoDate!=null){
            ...
            if(!isAnonymousInPlace){
                ...
                for(Group g : authorizedGroups){
                    ...
                    if(rp!=null) {
                        rp.update();
                        log.info("Resource Policy Resource_id " + dso.getID() + "resource_type " + dso.getTypeText() + " action " + rp.getActionText() + " Group " + rp.getGroup().getName() + "start date" + rp.getStartDate().toGMTString() + " EndDate " + rp.getEndDate().toGMTString());
                    }
                    else
                        log.info("No resource policy");
                ...
                // add policy just for anonymous
                 log.info("Just for AnonymousInPlace ");
