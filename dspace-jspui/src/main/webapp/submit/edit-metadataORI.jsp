...
<style>
input.ds-authority-value.orcid-control, input.ds-authority-value 
{
...
<%!
   ...
   // Render the choice/authority controlled entry, or, if not indicated,
    // returns the given default inputBlock
    StringBuffer doAuthorityORCID(PageContext pageContext, String fieldName,
            int idx, int fieldCount, String fieldInput, String authorityValue,
            int confidenceValue, boolean isName, boolean repeatable,
            Metadatum[] dcvs, StringBuffer inputBlock, int collectionID)
    {
        ...
        if (cam.isChoicesConfigured(fieldName))
        {
            ...
            { 
              ...
              sb.append("<input type=\"text\" value=\"").append(authorityValue!=null?authorityValue:"")
				  ...
				  .append("<input type=\"text\" value=\"").append(authorityValue!=null?"":"")
                  .append("\" id=\"orcid_").append(authorityName)
                  .append("\" name=\"orcid_").append(authorityName).append("\" class=\"ds-authority-value form-control orcid-control\"/>")
                ...
                //sb.append("<p>").append(authorityValue).append("</div>");
            }
      ...
      throws java.io.IOException
    {
      ...
      for (int i = 0; i < fieldCount; i++)
      {
         ...
         if ("authorLookup".equalsIgnoreCase(authorityType))
    	 {
			 ...
			 sb.append("<script>");
			 sb.append("function popitup(url) { newwindow=window.open(url,'name','height=400,width=400'); if (window.focus) {newwindow.focus()} return false;}");
			 sb.append("</script>");
			 
			 //sb.append("<script src='http://dspacetestes.bg.uc.pt:8080/jspui/scripts/theme.js'></script>");
			 sb.append("<script src='http://dspacetestes.bg.uc.pt:8080/jspui/scripts/person-lookup.js'></script>");
			 
			 
             sb.append("<p>authorLookupy</p>");
			 //botão	
			 // 				sb.append("<a href='http://dspacetestes.bg.uc.pt:8080/xmlui/teste.html' onclick='window.open(this.href)'</a>");
		
            
			//sb.append("<button onclick=\"javascript:AuthorLookup('http://dspacetestes.bg.uc.pt:8080/xmlui/choices/dc_contributor_author', 'dc_contributor_author', 1239);\" type=\"button\" name=\"lookup_dc_contributor_author\" class=\"ds-button-field ds-add-button btn btn-default \">");
			
			//sb.append("<a href=\"http://dspacetestes.bg.uc.pt:8080/jspui/teste2.html\" onclick=\"return popitup('http://dspacetestes.bg.uc.pt:8080/jspui/teste2.html')\"></a>");

		 
             sb.append(doAuthorityORCID(pageContext, fieldName, i, fieldCount, fieldName,
         ...
         sb.append("</div>");   
		 //orcid
			/* sb.append("<input type=\"text\" onchange=\"javascript: return DSpaceAuthorityOnChange(this, 'aspect_submission_StepTransformer_field_dc_contributor_author_confidence','');\" value=\"\" name=\"dc_contributor_author_authority\" readonly=\"\" class=\"ds-authority-value \">");*/
...
<%  //if not first page & step, show "Previous" button
		...
		//ARBT
        //String ale = "<script>window.alert(\"orcid value\");</script>";
        //out.write(ale);		
