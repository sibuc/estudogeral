...
public class SubmissionController extends DSpaceServlet
{
            ...
            SQLException, AuthorizeException
    {
        ...
        log.info("SubmissionController - doDSGet");
        // If resuming a workspace item
        if (workspaceID != null)
        {
            log.info("SubmissionController - workspaceID != null");
            try
            {
                ...
                {
                    log.info("getStepReached(si) >= si.getSubmissionConfig().getNumberOfSteps()");
                ...
                log.info("setBeginningOfStep(request, true);");
        ...
        {
            log.info("SubmissionController - workspaceID == null e workflowID != null");
            try
            {
        ...
        {
            log.info("SubmissionController - workspaceID == null e workflowID == null e !StringUtils.isEmpty(resumableFilename)");
        ...
        {
            log.info("SubmissionController - workspaceID == null e workflowID == null e StringUtils.isEmpty(resumableFilename)");
            log.info("SubmissionController - vai para doDSPost");
    ...
    protected void doDSPost(Context context, HttpServletRequest request,
            ...
            SQLException, AuthorizeException
    {
    	log.info("SubmissionController - iniciodoDSPost");
        ...
        {
            log.info("SubmissionController - contentType != null");
            try
            {
        ...
        log.info("SubmissionController - getSubmissionInfo");
        ...
        if (subInfo == null)
        {
            log.info("SubmissionController - subInfo == null");
        ...
        {
            log.info("SubmissionController - subInfo != null  e getSubmissionItem() == null");
         // we have just started this submission
            // (or we have just resumed a saved submission)
            //log.info("Coleção de doutoramentos ou provas de agregação: não é pedida a coleção");
            //String comm_doutoramentos = Community.find(context, ConfigurationManager.getIntProperty("comm.doutoramentos")).getHandle();
            ...
            //int id = 0;
            //String coll = null;
            //coll =  request.getParameter("collection");
            //log.info("Colecao escolhida ou nao - " + coll);
            //if ((coll!=null) && (coll!="")){
            //id = Integer.parseInt(coll);
            //log.info("id " + id);
            //    Collection current_coll = Collection.find(context, id);
            //    log.info("current_col=" + current_coll.getName());
            //    log.info("comm_dout.matches = " + comm_doutoramentos.matches(current_coll.getParentObject().getHandle()));
            //    log.info("comm_agreg.matches = " + comm_agregacao.matches(current_coll.getParentObject().getHandle()));
            //    if ((comm_doutoramentos.matches(current_coll.getParentObject().getHandle())) || (comm_agregacao.matches(current_coll.getParentObject().getHandle()))) {
            //        {
            //        log.info("Vai para doStep FIRST STEP");
            //        currentStepConfig = getCurrentStepConfig(request, subInfo);
            //        subInfo.setCollectionHandle(current_coll.getHandle());
                    // setBeginningOfStep(request, true);               
            ...
            //    else
            //    {
            //        log.info("Vai para doStep SELECT COLLECTION");
                    // setBeginningOfStep(request, true); 
          ...
          //  else
            ...
            log.info("SubmissionController - doStep - SELECT_COLLECTION");
            doStep(context, request, response, subInfo, SELECT_COLLECTION);
        }
        else
            ...
            AuthorizeException
    {
        ...
        if (subInfo.getSubmissionConfig() != null)
        {
            ...
            log.info("CurrentStep Config (number) - " + currentStepConfig.getStepNumber());
            log.info("Step Reached (number) - " + getStepReached(subInfo));
        ...
        try
        {
            log.info("CurrentStep Config (JSPUIClassName)" + currentStepConfig.getJSPUIClassName());
