    ...
    new Ajax.Request(contextPath+"/choices/"+field,
      {
        ...
        onSuccess: function(transport) {
		  ...
		  //ARBT
		  //form.elements['text1'].value = value;
          ...
          if (selectedByChoices >= 0)
            ...
            defaultSelected = 0;
		 else if ( defaultSelected == -1) //#ARBT
            defaultSelected = select.options.length-1;
...
function DSpaceChoicesAcceptOnClick ()
{
	...
	//ARBT
	var orcidInput = "orcid_" + this.form.elements['paramAuthorityInput'].value;
	
	
	//debug
	/*window.alert('orcidinput='+orcidInput);
    window.alert('confIndicatorID='+confIndicatorID);
	window.alert('orcid value='+this.form.elements['text3'].value);*/
	

	
    // default the authority input if not supplied.
    if (authorityInput.length == 0)
	alert (dspace_makeFieldInput(valueInput,'_authority'));
    ...
    if (valueInput.length > 0)
    {
        ...
        if (isName)
        {
			// When adding author
			//ARBT
			if (of.elements[dspace_makeFieldInput(valueInput,'_last')].value!="")
			{
            	of.elements[dspace_makeFieldInput(valueInput,'_last')].value = of.elements[dspace_makeFieldInput(valueInput,'_last')].value; //this.form.elements['text1'].value;
            	of.elements[dspace_makeFieldInput(valueInput,'_first')].value = of.elements[dspace_makeFieldInput(valueInput,'_first')].value //this.form.elements['text2'].value;
			}
			else
			{
				...
				//ARBT
    		 of.elements[orcidInput].value = this.form.elements['text3'].value;
			 of.elements[authorityInput].value = this.form.elements['text4'].value;
	
        }
        else
        { 
		
		//Se for para alterar o nome...
   		  of.elements[valueInput].value = this.form.elements['text1'].value + ", " + this.form.elements['text2'].value;	  
		  //ARBT
		  authorityInput =  authorityInput.replace("value_", "choice_");
		  of.elements[authorityInput].value = this.form.elements['text4'].value;
		}
		
			 
			 //window.alert('IDVAL value='+this.form.elements['text4'].value)
			 //alert ("Aqui1" + of.elements[authorityInput].value);
        ...
        if (authorityInput.length > 0 && of.elements[authorityInput] != null)
        {
            ...
            if (select.selectedIndex >= 0 && select.options[select.selectedIndex].authority != null)
            {
                ...
                //of.elements[authorityInput].value = "teste"; //authValue;
				 if (isName)
				 {
					of.elements[authorityInput].value = this.form.elements['text4'].value;
				 }
	...
	//of.elements[authorityInput].value = "teste";
	//alert ("final" + of.elements[authorityInput].value);
    window.close();
    return false;
}
