...
public class BrowseListTag extends TagSupport
{
    ...
    static
    {
      //  log.info("BrowseListTag");
        getThumbSettings();
     //   log.info("Apos Get Thumb settings");
        showAccessIcon = ConfigurationManager.getBooleanProperty("webui.browse.accessicon.show");
       
      if (showAccessIcon) {
        if (showThumbs)
        {
            DEFAULT_LIST_FIELDS = "thumbnail, dc.date.issued(date), dc.title, dc.contributor.*, accessicon";
            DEFAULT_LIST_WIDTHS = "*, 130, 60%, 40%, 30";
        }
        else
        {
            DEFAULT_LIST_FIELDS = "dc.date.issued(date), dc.title, dc.contributor.*, accessicon";
            DEFAULT_LIST_WIDTHS = "130, 60%, 40%, 30";
        }
      }
        else {
    ...
    public BrowseListTag()
    {
        ...
        super();
        // log.info("BrowseListTag:super");
    }

    public int doStartTag() throws JspException
    {
        // log.info("BrowseListTag:doStartTag");
        ...
        try
        {
            ...
            if (widthArr.length > 0 && widthArr.length == fieldArr.length && !linkToEdit)
            {
                ...
                for (int w = 0; w < widthArr.length; w++)
                {
                    ...
                    {
                      if (fieldArr[w].equals("accessicon"))
                       { 
                            out.print(noAccessIconMaxWidth);
                       }
                      else
                      {
            ...
            for (int i = 0; i < items.length; i++)
            {
                ...
                for (int colIdx = 0; colIdx < fieldArr.length; colIdx++)
                {
                    ...
                    else if (field.equals("accessicon")) {
                        metadata = getAccessIconMarkup(hrq, items[i]);
                    }
                    ...
                    {
                      //  log.info("metadataArray.length>0");
                        ...
                        {
                         //   log.info("TitleField e isWithdrawn");
                        ...
                        {
                          //  log.info("BrowseListTag:format all other fields");
                        	...
                        	//	log.info("Limiting output of field " + field + " to " + Integer.toString(loopLimit) + " from an original " + Integer.toString(metadataArray.length));
                        	}
                            ...
                            for (int j = 0; j < loopLimit; j++)
                            {
                            	String startLink = "";
                            	String endLink = "";
                             //   log.info("Ciclo for de BrowseListTag");
                            	if (!StringUtils.isEmpty(browseType[colIdx]) && !disableCrossLinks)
                            	{
                                                ...
                                                .getMinConfidence(metadataArray[j].schema, metadataArray[j].element, metadataArray[j].qualifier))
                                    {
                                        ...
                                        startLink = startLink + "&amp;" + argument + "=" + URLEncoder.encode(value,"UTF-8");
                                    }
                                        argument = "value";
  ...
  /* generate the (X)HTML required to show the access icon        		 */
    /* access icon shows a locker if the full text is not available       	 */
    /* default image is textintegrallock.png existent in           		 */
    /* <dspace-source>/dspace-jspui/dspace-jspui-webapp/src/main/webapp/image */

    private String getAccessIconMarkup(HttpServletRequest hrq, BrowseItem browseitem)
            throws JspException
    {
        // log.info("getAccessIconMarkup");
    	try
    	{
            Context c = UIUtil.obtainContext(hrq);
            
            Item item = Item.find(c, browseitem.getID());

            noAccessIconMaxWidth= ConfigurationManager
                        .getIntProperty("webui.browse.noaccessicon.maxwidth");

           if (item == null)
       	{
    	  //  log.info("getAccessIconMarkup - item is null");	
            return "";
    	}

    	// now go sort out the access to bitstreams

    	// if there's no original, there is no thumbnail
    	Bundle[] original = item.getBundles("ORIGINAL");
        if (original.length == 0)
        {
          //  log.info("getAccessIconMarkup - bundle ORIGINAL is null");	
            return "";
        }
        ...
        if (original[0].getBitstreams().length > 0)
        {
           ...
           // log.info("getAccessIconMarkup - bitstreams.length > 0");
            Bitstream[] bitstreams = original[0].getBitstreams();

            for (int i = 0; (i < bitstreams.length) && !access; i++)
            {

                    access = AuthorizeManager.authorizeActionBoolean(c,bitstreams[i],Constants.READ,true);
             //    log.info("getAccessIconMarkup - access" + access);
       ...
       if (!access)
       {
        // log.info("getAccessIconMarkup - no access");
        img = hrq.getContextPath() + "/image/" + ConfigurationManager.getProperty("webui.browse.noaccessicon.image");
        alt = LocaleSupport.getLocalizedMessage(pageContext, "browselist.noaccess");
        }
       else {
          //  log.info("getAccessIconMarkup - access");
            img = hrq.getContextPath() + "/image/" + ConfigurationManager.getProperty("webui.browse.accessicon.image");
            alt = LocaleSupport.getLocalizedMessage(pageContext, "browselist.access");
        }
        thumbFrag.append("<img src=\"")
                     .append(img)
                     .append("\" alt=\"")
