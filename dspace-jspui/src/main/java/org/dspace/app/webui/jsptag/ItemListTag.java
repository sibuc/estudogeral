                ...
                for (int colIdx = 0; colIdx < fieldArr.length; colIdx++)
                {
                    ...
                    if (metadataArray.length > 0)
                    {
                        ...
                        else
                        {
                            ...
                            // log.info("ItemListTag: authorfield");
                            ...
                            for (int j = 0; j < loopLimit; j++)
                            {
                                ...
                                if (!StringUtils.isEmpty(browseType[colIdx]) && !disableCrossLinks)
                                {
                                    ...
                                    // log.info("ItemListTag: startLink 1- " + startLink);
                                                ...
                                                .getMinConfidence(metadataArray[j].schema, metadataArray[j].element, metadataArray[j].qualifier))
                                    {
                                        ...
                                        // log.info("ItemListTag: startLink 2- " + startLink);
                                    }
                                    argument = "value";
                                    value = metadataArray[j].value;
                                    startLink = startLink + "&amp;" + argument + "=" + URLEncoder.encode(value,"UTF-8");
                                    // log.info("ItemListTag: startLink 3- " + startLink);
                                    if (viewFull[colIdx])
                                    {
                                        argument = "vfocus";
                                        startLink = startLink + "&amp;" + argument + "=" + URLEncoder.encode(value,"UTF-8");
                                        // log.info("ItemListTag: startLink 4- " + startLink);
                                    }
                                    if (metadataArray[j].language != null)
                                    {
                                        startLink = startLink + "&amp;" +
                                            argument + "_lang=" + URLEncoder.encode(metadataArray[j].language, "UTF-8");
                                        // log.info("ItemListTag: startLink 5- " + startLink);
                                    }

                                    if ("authority".equals(argument))
                                    {
                                        startLink = startLink + "\" class=\"authority " +browseType[colIdx] + "\">";
                                        // log.info("ItemListTag: startLink 6- " + startLink);
                                    }
                                    else
                                    {
                                        startLink = startLink + "\">";
                                        // log.info("ItemListTag: startLink 7- " + startLink);
                                    }
                                    endLink = "</a>";
                                }
                                sb.append(startLink);
