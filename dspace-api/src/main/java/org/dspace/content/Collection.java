    ...
    public static Collection[] findAll(Context context, Integer limit, Integer offset) throws SQLException
    {
        ...
        try
        {
            ...
            while (tri.hasNext())
            {
                ...
                // First check the cache
                Collection aCollection = (Collection) context.fromCache(
                        Collection.class, row.getIntColumn("collection_id"));

                if (aCollection == null)
                {
                    aCollection = new Collection(context, row);
                }
                if (AuthorizeManager.authorizeActionBoolean(context, aCollection, Constants.READ))
                {
