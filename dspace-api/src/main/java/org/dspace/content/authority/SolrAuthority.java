...
public class SolrAuthority implements ChoiceAuthority {
    ...
    public Choices getMatches(String field, String text, int collection, int start, int limit, String locale, boolean bestMatch) {
        if(limit == 0)
            limit = 10;
        log.info("SolrAuthority: getMatches");
        ...
        try {
            ...
            log.info("SolrAuthority: getMatches getSearchService " + getSearchService().toString());
    ...
    protected void addExternalResults(String text, ArrayList<Choice> choices, List<AuthorityValue> alreadyPresent, int max) {
         log.info("SolrAuthority:addExternalResults");
        if(source != null){
            try {
    ...
    private String toQuery(String searchField, String text) {
        log.info("SolrAuthority:toQuery");
    ...
    public Choices getBestMatch(String field, String text, int collection, String locale) {
        log.info("SolrAuthority:getBestMatch");
    ...
    public String getLabel(String field, String key, String locale) {
        log.info("SolrAuthority:getLabel");
        try {
  ...
  //          QueryResponse searchResponse = getSearchService().search(queryArgs);
            ...
            log.info("GET LABEL: field " + field);
            log.info("GET LABEL: requesting label for key " + key + " using locale " + locale);
            SolrQuery queryArgs = new SolrQuery();
            queryArgs.setQuery("id:" + key);
            queryArgs.setRows(1);
            log.info("GET LABEL: queryArgs " + queryArgs.getQuery());
    ...
    public static AuthoritySearchService getSearchService() {
        ...
        log.info("SolrAuthority:getSearchService");
        org.dspace.kernel.ServiceManager manager = dspace.getServiceManager();
                log.info("getSearchService AuthoritySearchService.class.getName()" + AuthoritySearchService.class.getName());
    ...
    public void addExternalResultsInNextMatches() {
        log.info("SolrAuthority:addExternalResultsInNextMatches");
