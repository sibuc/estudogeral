...
import org.dspace.content.solrOrcid;
...
public class AuthoritySolrServiceImpl implements AuthorityIndexingService, AuthoritySearchService {
    ...
    public void indexContent(AuthorityValue value, boolean force) {
        ...
        try{        
            ...
            context = new Context();
            String id = doc.get("id").getValue().toString();
            String orcid = doc.get("orcid_id").getValue().toString();
            log.info("write solr authority index");
            log.info("id:" + id );
            log.info("orcid:" + orcid);
            
            writeDocument(doc);
            solrOrcid s = solrOrcid.create(context, id, orcid);
        ...
        try {
            ...
            log.error("Error while writing authority value to orcid_authority table", e);
