...
//               Remove from indices
     ...
     * @throws IOException
     */
    public void move_thesis_dissertations (Collection from, Collection to, Calendar embargoEndDate ) throws SQLException, AuthorizeException, IOException
    {
        // Check authorisation on the item before that the move occur
        // otherwise we will need edit permission on the "target collection" to archive our goal
        // only do write authorization if user is not an editor
        ...
        if (isOwningCollection(from))
        {
            ...
            log.info("Data atual:" + formatter.format(cal.getTime()));
            log.info("Embargo end Date:" + formatter.format(embargoEndDate.getTime()));
            log.info("Embargo ultrapassado?" + embargoEndDate.getTime().before(cal.getTime()));
            if ((embargoEndDate == null) || embargoEndDate.getTime().before(cal.getTime()))
            {
                log.info(LogManager.getHeader(ourContext, "move_item",
                         "Updating item with inherited policies"));
                inheritCollectionDefaultPolicies(to);
            ...
            {
               log.info(LogManager.getHeader(ourContext, "move_item",
                         "Updating item with new policies"));
                // retira permissões devido a embargo
                List<ResourcePolicy> permissoes = new ArrayList<ResourcePolicy>();
                ResourcePolicy e = ResourcePolicy.create(ourContext);
                EPerson Ep = this.getSubmitter();
                e.setEPerson(Ep);
                e.setStartDate(cal.getTime());
                e.setEndDate(embargoEndDate.getTime());
                e.setAction(Constants.READ);
                permissoes.add(e);                
                Bundle[] bundles = this.getBundles("ORIGINAL");
                ourContext.turnOffAuthorisationSystem();
                for (Bundle bdl : bundles) {
                    AuthorizeManager.removeAllPolicies(ourContext, bdl);
                    bdl.replaceAllBitstreamPolicies(permissoes);
                    AuthorizeManager.addPolicies(ourContext, permissoes, bdl);
                }
                
                this.replaceAllItemPolicies(permissoes);
                ourContext.restoreAuthSystemState();
            }
            // Update the item
            ourContext.turnOffAuthorisationSystem();
