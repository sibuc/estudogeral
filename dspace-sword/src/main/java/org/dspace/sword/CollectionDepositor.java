...
public class CollectionDepositor extends Depositor
{
			...
			throws SWORDErrorException, DSpaceSWORDException
	{
		...
		Integer workflowItemId = result.getItem().getID();
		Integer itemId = result.getItem().getID();
		Integer collectionId = collection.getID();
		Integer oldstate = 20;
		Integer sessionId = 0;
		String remoteaddr = deposit.getIPAddress();
		
		try{
			insertAdvanceWorkflow(workflowItemId, itemId, collectionId, oldstate, context, remoteaddr);	
		}
