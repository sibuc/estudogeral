...
public class InstallItem
{
    ...
    {
        log.info("InstallItem.installItem(2)");
    ...
    {
        log.info("InstallItem.installItem(3)");
        Item item = is.getItem();
        IdentifierService identifierService = new DSpace().getSingletonService(IdentifierService.class);
        try {
            if(suppliedHandle == null)
            {   
                log.info("InstallItem.installItem(3).identifierService.register(c,item)");
                identifierService.register(c, item);
            }else{
                log.info("InstallItem.installItem(3).identifierService.register(c,item,suppliedhandle");
        ...
        log.info("InstallItem.installItem(3):depois de populate Metadata - vai para finish item");
    ...
    private static void populateMetadata(Context c, Item item)
        throws SQLException, IOException, AuthorizeException
    {
        log.info("InstallItem.populateMetadata");
    ...
    private static Item finishItem(Context c, Item item, InProgressSubmission is)
        throws SQLException, IOException, AuthorizeException
    {
        log.info("INICIO InstallItem.finishItem");
        ...
        log.info("FIM InstallItem.finishItem");
        return item;
    }
