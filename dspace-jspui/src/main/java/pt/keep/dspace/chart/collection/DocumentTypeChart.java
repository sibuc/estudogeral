...
public class DocumentTypeChart extends AbstractCollectionChart {
	private class Data {
		public int count;
		public String type;
	}
	
    private static Logger log = Logger.getLogger(DocumentTypeChart.class);
	private String query (Collection c) {
		try {
			return "SELECT COUNT(t.item_id) as cnt, t.typea as typea "+
			"FROM (SELECT i.item_id, (SELECT m.text_value FROM metadatavalue m WHERE m.metadata_field_id="+ReportDatabaseManager.getFieldID(super.getContext(),"dc.type")+" AND m.item_id=i.item_id) as typea "+ 
			"FROM item i " +
			"WHERE (" +
			"(i.item_id IN (SELECT tmp.item_id FROM collection2item tmp WHERE tmp.collection_id IN ("+c.getID()+")))" +
			")) t "+
	...
	private List<Data> retrieve (Collection c) {
		...
		try {
			...
			while (iterator.hasNext()) {
				...
				nod.type = row.getStringColumn("typea"); //rs.getString("typea");
				result.add(nod);
			}
		} catch (SQLException e) {
			log.debug(e.getMessage(), e);
	...
	private String getData (String varname, Collection c, PageContext page) {
		List<Data> list = this.retrieve(c);
		List<String> join = new ArrayList<String>();
		for (Data nod : list) {
			String t = LocaleSupport.getLocalizedMessage(page, "stats.reporting.field.doctype."+nod.type);
			if (t.contains("???"))
				join.add("{ label: \""+nod.type+"\", data: "+nod.count+" }");
			else
